...
npm run check        # פותח כל עמוד בדפדפן אמיתי ונכשל אם משהו שבור
...
לפני `npm run check` בפעם הראשונה צריך `npx playwright install chromium`.

## בדיקת תקינות

כל דחיפה מריצה את `npm run check` ב-GitHub Actions, ו**הדיפלוי מחכה לה** - אם היא נכשלת, שום דבר לא עולה. הבדיקה מרימה את `dist/` בשרת אמיתי, פותחת כל עמוד ב-Chromium ונכשלת על: קוד תשובה שאינו 200, שגיאת קונסול או חריגה שמקורה אצלנו, בקשה שנכשלה לקובץ שלנו, או אלמנט חסר (קנבס באתרי התלת-ממד, `#root` מאוכלס באפליקציות React). בנוסף היא בודקת בדפדפן שכל לינק בעמוד הבית מחזיר 200 ושלכל קלף יש תמונת תצוגה, ושה-zip של התוסף מתחיל ב-`PK`.

רשימת העמודים נגזרת מ-`tools/build/targets.mjs` ומ-`projects.js`, אז אפליקציה חדשה נבדקת מאליה ואפליקציה שנמחקה מפסיקה להיבדק. כשלים של Google Fonts או של ה-CDN שממנו נטען three.js **לא** מכשילים את הבנייה - הם מחוץ לשליטתנו, ובדיקה שנצבעת אדום מסיבות זרות היא בדיקה שמפסיקים להאמין לה.
